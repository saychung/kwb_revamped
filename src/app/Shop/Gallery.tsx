    ...
    return (
        <div className="h-fit w-screen grid place-content-center pt-10 pb-20"> 
              ...
              {paintings.map((painting:any, index: number) => (
                  <div key={index} data-aos='fade-right' data-aos-delay={delay=delay+50} className="backdrop-blur-xl  group hover:cursor-pointer rounded-md bg-black/20 " >
                    <Link  href={`/Shop/${painting.slug}`} className="grid grid-rows-2 h-full w-[200px] relative">
                    ...
                    <div className= "row-span-1 pl-2 pb-2 h-full text-[13px] text-white">
                    ...
                    <p className="absolute top-0 right-[4px] hidden group-hover:inline text-green-300 text-[50px]">{`.`}</p>
                    </Link>
                    </div>
